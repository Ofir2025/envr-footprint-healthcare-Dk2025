...
def t_scenarios() -> Table | None:
    """Table 12, the mitigation ladder against the regional target.

    The builder lives in :mod:`analysis.mitigation_scenarios`, beside the layer
    it reads, because that layer is withheld from the revision and from the
    published branch (see :mod:`analysis.gold_scope`).

    Returns
    -------
    Table or None
        None where the scenario layer is not reported.
    """
    if not gold_scope.reported("18_mitigation_scenarios"):
        return None
    from analysis.mitigation_scenarios import table_target_consistency
    return table_target_consistency()


def t_burden_shift() -> Table | None:
    """Table 13, relative change in every category for every lever.

    Returns
    -------
    Table or None
        None where the scenario layer is not reported; the builder lives in
        :mod:`analysis.mitigation_scenarios` for the reason given in
        :func:`t_scenarios`.
    """
    if not gold_scope.reported("18_mitigation_scenarios"):
        return None
    from analysis.mitigation_scenarios import table_burden_shift
    return table_burden_shift()
...
def main() -> None:
    ...
    for build in BUILDERS:
        ...
        if t is None:
            ...
            print(f"  skip {build.__name__}: its layer is not in this copy or is withheld")
            continue
