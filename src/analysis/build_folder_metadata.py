...
#: `docs/methods/star_schema.sql`; that file is NOT
#: `docs/methods/denmark_healthcare_star_schema.sql`, an earlier, different
#: draft schema (it declares `dim_geography`, `dim_time` and others that do
#: not exist on disk) - `build_manifest.py`'s star row cites the same, correct
#: file, and the two must not be confused.
