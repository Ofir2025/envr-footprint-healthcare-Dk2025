...
SUPERSEDED_TEXT: tuple[tuple[str, str], ...] = (
    ...
    ("47.28 %", "2019_uncorrected's transport share on AR4; it is 46.85 % of "
                "layer 01's grand total and 46.87 % of layer 02's, which "
                "excludes the self-supply loop"),
...
HISTORICAL_DOCS: frozenset[str] = frozenset({
    ...
    # Generated by analysis.build_folder_metadata from a note that exists to
    # record the AR4-to-AR6 move for this folder: it states 4,054.772061 kt as
    # what the total *was* until the 2016 background was rebuilt. A ban on that
    # value is exactly what the note is about, so the note is not a stale claim.
    "data/gold/results/02_scopes_wood_hertwich/2019c/readme.md",
})
...
#:
#: ``data/gold/results`` is here because a gold folder's ``readme.md`` is a
#: published deliverable, not commentary, and because most of them are
#: *generated* by :mod:`analysis.build_folder_metadata` from hand-written notes
#: held in that module. A stale number in one of those notes is rendered into
#: gold on every build while never appearing in a document this check used to
#: read, which is how ``2019_uncorrected``'s pre-AR6 transport share of 47.28 %
#: survived in ``02_scopes_wood_hertwich/2019a/readme.md`` after the value it
#: quotes had been banned here. Scanning the rendered output rather than the
#: module is deliberate: what a reader sees is what has to be right, and a note
#: that never reaches a readme is not a published claim.
#:
#: The repository readme is here for the same reason. It now publishes the
#: climate footprint, the national share and the transport share of all eight
#: variants, so it states more current numbers than most documents in
#: ``docs/revision``.
#: Only ``readme.md`` is read under gold, never ``data_dictionary.md``. A data
#: dictionary is machine output: its example cells are drawn from the live table,
#: so it cannot state a superseded value, and its long float literals produce
#: substring collisions instead -- ``"13.19"`` matches inside
#: ``613.1946464775333``.
CLAIM_TREES: tuple[str, ...] = ("docs/revision", "docs/methods/replications.md",
                                "docs/methods/methods.md", "figures/manuscript",
                                "data/gold/results/**/readme.md", "readme.md")
...
def c6b_superseded(results: list[dict[str, Any]]) -> None:
    ...
    for tree in CLAIM_TREES:
        ...
        # An entry that is itself a glob is used as written, so a tree can be
        # narrowed to one filename without a second constant.
