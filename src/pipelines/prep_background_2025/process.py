# -*- coding: utf-8 -*-
"""
The environmental footprint of the Dutch healthcare sector: beyond environmental impact (in press)
Steenmeijer MA, Rodrigues JFD, Zijp MC, Waaijers-van der Loop SL
The Lancet Planetary Health

Tasks exiobase_3_7-process.py:

    1. Assemble MRIO
    2. Merge in dictionary and store as pickle

@author: Joao F. D. Rodrigues
minor additions by Michelle A. Steenmeijer
"""

import numpy as np
import os
import time
import pickle as pkl
import sys
from paths import MRIO_DIR
from analysis.constants import (EXIOBASE_RELEASE, RELEASE_TAG, table_year,
                                write_release_sidecar)
np.set_printoptions(precision = 2)

tstart = time.time()
# Release-tagged stem: mrio2016.pkl is v3.8.2's 2016 table, mrio2016_v3_7.pkl
# is v3.7's, and neither can be mistaken for the other.
year = os.environ.get('HC_BACKGROUND_YEAR', '') or table_year()
stem = year + RELEASE_TAG.get(EXIOBASE_RELEASE, f'_{EXIOBASE_RELEASE}')
print(f"release {EXIOBASE_RELEASE} | background stem {stem}")
##############################################
##############################################
#TASK 1: Load Exiobase v3.7 and calibrate MRIO


##############################################
# Folder settings: Change to reflect the location in your computer relative to the current working directory (run os.getcwd() to find out whatthat is)
# Set working directory to envr-footprint-healthcare2025 folder
print("Starting to read files..\n")
mrio_dir = str(MRIO_DIR) + os.sep


##############################################
#Load MRIO

tstart = time.time()

mrio_str = 'exio'+ stem +'.pkl'  
pkl_in = open(mrio_dir + mrio_str,"rb")
mrio = pkl.load(pkl_in)
pkl_in.close()

mrio_str = 'leontief'+ stem +'.pkl'  
pkl_in = open(mrio_dir + mrio_str,"rb")
L = pkl.load(pkl_in)
pkl_in.close()

tend = time.time()
print('Done reading in %5.2f s\n'% (tend - tstart))
tstart = time.time()


# Number of..
nr = mrio['label']['region'].count().iloc[0]  # number of regions
ns = mrio['label']['industry'].count().iloc[0]  # number of sectors

# Calculation x (total output)
x = np.dot(L, mrio['Y'].sum(1).reshape((nr*ns,1)))    # x = L*y
# Calculation Z matrix (intermediate demand matrix/transaction matrix)
Z = np.dot(mrio['A'], np.diag(x[:,0]))  # Z = A*diagn(x)

tend = time.time()
print('Done calculating interindustry transactions in %5.2f s\n'% (tend - tstart))
tstart = time.time()

mrio['Z'] = Z
mrio['x'] = x

#############################################
# save to pickle
mrio_str = 'mrio'+ stem +'.pkl'  
pkl_out = open(mrio_dir + mrio_str,"wb")
pkl.dump(mrio, pkl_out)
pkl_out.close()

# Mirror the release the arrays actually came from beside the pickle, so
# constants.model_label() reads it from the background rather than from a
# constant. A pickle built before this field existed leaves the sidecar to be
# derived from the stem, which carries the release tag anyway.
write_release_sidecar(mrio_dir, stem,
                      mrio.get('release', EXIOBASE_RELEASE),
                      str(mrio.get('source', '')))

tend = time.time()
print('Done writing in %5.2f s\n'% (tend - tstart))
tstart = time.time()
