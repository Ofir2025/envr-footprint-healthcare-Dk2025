# 01_eriksen_replication/2019a - data dictionary
...
| `value` | measure | float64 | kt CO2eq | 0.0002791482599497 |
...
| `value` | measure | float64 | varies by row | 3210.723786854978 |
...
| `value` | measure | float64 | varies by row | 6327.321490938661 |
...
| `value` | measure | float64 | varies by row | 23.799159757093484 |
| `share_of_total_pct` | measure | float64 | % | 33.12428718036501 |
...
| `value` | measure | float64 | varies by row | 25.459380302399236 |
| `share_pct` | measure | float64 | % | 35.43502515123223 |
...
| `value` | measure | float64 | varies by row | 59.43816773486931 |
| `share_pct` | measure | float64 | % | 82.7275818818642 |
...
| `indicator` | dimension | str |  | blue_water_consumption |
| `unit` | dimension | str |  | Mm3 |
| `value` | measure | float64 | varies by row | 6.553700994572583 |
| `rank` | measure | int64 |  | 1 |
| `share_pct` | measure | float64 | % | 9.150676543818818 |
| `mrio_coverage_pct` | measure | float64 | % | 99.68236561925956 |
...
| `value` | measure | float64 | varies by row | 31.553459104853527 |
| `share_pct` | measure | float64 | % | 43.916921924588046 |
...
| `Global warming (ktCO2eq)` | measure | float64 |  | 438.16362228419064 |
| `Material extraction (kt)` | measure | float64 |  | 191.30637177396588 |
| `Blue water consumption (Mm3)` | measure | float64 |  | 20.359050727749743 |
| `Land use (km2)` | measure | float64 |  | 1474.3946286297012 |
| `Waste generation (kt)` | measure | float64 |  | 38.90785967364414 |
...
|:---|:---|:---|:---|:---|
| `Contribution` | dimension | str |  | Food and food services |
| `Global warming (ktCO2eq)` | measure | float64 |  | 5.374464985709317 |
| `Material extraction (kt)` | measure | float64 |  | 3.1687023532020606 |
| `Blue water consumption (Mm3)` | measure | float64 |  | 28.33625430093593 |
| `Land use (km2)` | measure | float64 |  | 32.445872729477564 |
| `Waste generation (kt)` | measure | float64 |  | 11.684933941093869 |
...
| `Global warming (ktCO2eq)` | measure | float64 |  | 500.8457106841507 |
| `Material extraction (kt)` | measure | float64 |  | 19.912271805337 |
| `Blue water consumption (Mm3)` | measure | float64 |  | 59.4381677348693 |
| `Land use (km2)` | measure | float64 |  | 4441.487075597678 |
| `Waste generation (kt)` | measure | float64 |  | 56.06685808956781 |
...
|:---|:---|:---|:---|:---|
| `Hotspot` | dimension | str |  | Agricultural sector |
| `Global warming (ktCO2eq)` | measure | float64 |  | 6.14331632845776 |
| `Material extraction (kt)` | measure | float64 |  | 0.3298168374742907 |
| `Blue water consumption (Mm3)` | measure | float64 |  | 82.72758188186413 |
| `Land use (km2)` | measure | float64 |  | 97.74040245819069 |
| `Waste generation (kt)` | measure | float64 |  | 16.8381797034461 |
...
| `Global warming (ktCO2eq)` | measure | float64 |  | 294.06401740610966 |
| `Material extraction (kt)` | measure | float64 |  | 32.87081939328065 |
| `Blue water consumption (Mm3)` | measure | float64 |  | 0.228214159913094 |
| `Land use (km2)` | measure | float64 |  | 2.1400062698201463 |
...
|:---|:---|:---|:---|:---|
| `Region` | dimension | str |  | Unallocated |
| `Global warming (ktCO2eq)` | measure | float64 |  | 3.606955677578107 |
| `Material extraction (kt)` | measure | float64 |  | 0.5444556906146012 |
| `Blue water consumption (Mm3)` | measure | float64 |  | 0.3176343807404349 |
| `Land use (km2)` | measure | float64 |  | 0.047093478043528 |
| `Waste generation (kt)` | measure | float64 |  | 0.0 |
...
| `indicator` | dimension | str |  | climate_change |
| `unit` | dimension | str |  | kt CO2eq |
| `value` | measure | float64 | varies by row | 1501.7446975739383 |
| `analysis_year` | measure | int64 |  | 2019 |
| `model` | dimension | str |  | EXIOBASE v3.7 IOT_2016_ixi |
...
| `value` | measure | float64 | kt CO2eq | 0.034085349708692 |
| `indicator` | dimension | str |  | climate_change |
| `unit` | dimension | str |  | kt CO2eq |
| `component_type` | dimension | str |  | MRIO supply-chain node |
| `analysis_year` | measure | int64 |  | 2019 |
...
| `value` | measure | float64 | varies by row | 5905.564420238479 |
...
| `unit` | dimension | str |  | kt CO2eq |
| `producing_world_region` | dimension | str |  | Denmark |
| `value` | measure | float64 | varies by row | 4136.726537656629 |
...
| `unit` | dimension | str |  | Mm3 |
| `origin` | dimension | str |  | domestic |
| `value` | measure | float64 | varies by row | 7.547016326189557 |
| `share_of_total_pct` | measure | float64 | % | 10.5041328641485 |
...
| `value` | measure | float64 | kt CO2eq per MEUR | 0.795071686487501 |
| `indicator` | dimension | str |  | climate_change |
...
| `expenditure_meur` | measure | float64 | varies by row | 41.47582895213488 |
| `footprint` | measure | float64 | varies by row | 1125.450898219894 |
...
| `value` | measure | float64 | varies by row | 3.402960830745904 |
| `expenditure_meur` | measure | float64 | varies by row | 652.648520413086 |
| `footprint` | measure | float64 | varies by row | 2220.93735121 |
...
| `kt_CO2eq` | measure | float64 |  | 139.4309722503604 |
...
| `Climate change (kt CO2eq)` | dimension | str |  | 8,153 (100·0%) |
| `Material extraction (kt)` | dimension | str |  | 6,037 (100·0%) |
| `Blue water consumption (Mm3)` | dimension | str |  | 72 (100·0%) |
| `Land use (km2)` | dimension | str |  | 4,544 (100·0%) |
| `Waste generation (kt)` | dimension | str |  | 333 (100·0%) |
...
|:---|:---|:---|:---|:---|
| `Unnamed: 0` | dimension | str |  | Total |
| `Global warming (ktCO2eq)` | measure | float64 |  | 8152.692843832223 |
| `Material extraction (kt)` | measure | float64 |  | 6037.372730216989 |
| `Blue water consumption (Mm3)` | measure | float64 |  | 71.84806612593555 |
| `Land use (km2)` | measure | float64 |  | 4544.166960533617 |
| `Waste generation (kt)` | measure | float64 |  | 332.97457965775965 |
...
| `Healthcare footprint` | measure | float64 |  | 8152.692843832223 |
| `National consumption footprint` | measure | float64 |  | 79040.33438684596 |
| `Healthcare share of national consumption footprint (%)` | measure | float64 | % | 10.31459811889284 |
