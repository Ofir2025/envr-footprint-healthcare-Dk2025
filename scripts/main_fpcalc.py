...
Tasks:
    1. Prepare objects required for running tool 
    2. Prepare template for specifying parameters
    3. Copy code and instruction files
    4. Compress everything and copy to root folder

@author: Joao F. D. Rodrigues
Updated and revised by Michelle Steenmeijer
"""
import pandas as pd
import numpy as np
import numpy.matlib
...
# 2A) Retrieve CBS data for 2016. If not needed to update, comment out and only run next line
...
##############################################
# 4)  Functions for footprint calculation
##############################################
...
def df_fromarray(arrs_hotspot, char_labels, multiindex):
    l_df = []
...
df_contrib = df_fromarray(array_contrib, char_labels, multiindex)
df_hotspot = df_fromarray(array_hotspot, char_labels, multiindex)
...
df_mult = pd.concat([Y_df.iloc[:,:-3], df_contrib[0].iloc[:163 * 49, 2:]], axis = 1)
df_mult = df_mult[df_mult['Total (MEUR)'] != 0]  # cannot divide by zero
for x in cols_impcat:
    df_mult[x] = df_mult[x].astype('float')

# Aggregate to later get the weighted average per (aggregated) product group
mult_aggsec_aggreg = df_mult.groupby(['RegName', 'SAggDescription'])[['Total (MEUR)', 'Global warming (ktCO2eq)', 'Material extraction (kt)', 'Blue water consumption (Mm3)','Land use (km2)', 'Waste generation (kt)']].sum()
mult_aggsec = df_mult.groupby(['SAggDescription'])[cols_impcat + ['Total (MEUR)']].sum()
mult_allsec = df_mult.groupby(['SecName'])[cols_impcat + ['Total (MEUR)']].sum()

#!!! !!! write as for statement
def agg_intensity(df, col_Y):
    for x in cols_impcat:
        df[str(x)+'/MEUR'] = df[x] / df[col_Y]
        del df[x]
    return df

# Calculate weighted averages
mult_all = agg_intensity(df_mult, 'Total (MEUR)')
mult_aggsec_aggreg = agg_intensity(mult_aggsec_aggreg, 'Total (MEUR)')
mult_aggsec = agg_intensity(mult_aggsec, 'Total (MEUR)')
mult_allsec = agg_intensity(mult_allsec, 'Total (MEUR)')
...
df_c_aggsec.to_excel(writer, sheet_name='allsec')
