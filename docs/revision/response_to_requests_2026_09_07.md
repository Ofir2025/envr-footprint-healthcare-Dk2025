...
Karliner et al. 2019 Appendix A (WIOD, USD), Lenzen et al. 2020 and Pichler et
al. 2019 (both USD) — which are labelled as those studies' own units.
...
Read Wood & Hertwich (2018) and Cabernard et al. (2019 + SI, 2022 + SI) and
...
- **Scope 2 now uses the energy-block inverse** `L_EE = (I_EE − A_EE)⁻¹` over
  ...
  refining and grid hardware correctly stay in Scope 3 (GHG Protocol category 3).
  The previous full-`L` version is kept as a reported sensitivity (401.5 vs
  404.2 kt).
- **Self-supply loop:** because the services component is `y = A[:,h]·E_H`, the
  ...
  own direct emissions, overlapping the national-accounts Scope 1. Removed.
  I first tried zeroing the demand element and **rejected that**: it also
  deletes the legitimate upstream chain of internally traded health services
...
Audited our first version against the IEooc `Methods5_Exercise4b` notebook and
the published literature, then rebuilt it. Findings and fixes:
...
silently zeroes 109 cells carrying 53.8 % of German final demand). Their exercise
...
Tested every option with real data rather than reasoning in the abstract.
...
submission (95.7 % agreement with `env_wasgen` for services), so this is
Eurostat-consistent rather than an alternative to it.

## 6. Childcare and the scope boundary — should we include it, what do others do?

Implemented as a switch (`HC_SCOPE`) and run for all five indicators:
...
and P to water. This matches Lenzen's stressor families and gives the inventory
side of an Eckelman-style health-damage analysis (no DALY conversion applied —
that needs an LCIA model with defensible endpoint factors).

## 7. FIGARO for Denmark 2022

Downloaded and used. Eurostat's official FIGARO-based GHG footprint
