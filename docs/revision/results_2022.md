...
the correction of the submitted manuscript's transport finding (one of this
...
1. [The corrected transport share](#the-corrected-transport-share)
...
6. [Mitigation scenarios](#mitigation-scenarios)
...
to the Danish health-care footprint (39 % of the input-output component by
purchase, 46 % by producing sector), does **not hold after correction**. It was a faithful computation on a Danish EXIOBASE block that
...
moves the share from 46 % to 15.6 %, and states what the paper should say now
...
**Effect:** transport falls from 40.1 % to **19.5 %** of the health-care climate
supply-chain footprint (the 3,415 kt MRIO component; 15.6 % of the 4,267 kt total
once the domestic bottom-up items are included); the Danish sea-transport node
falls from 857 kt to **53 kt**; the Danish national footprint falls from 86.3 Mt
to **77.2 Mt**.
...
flowchart LR
    ...
    EFF["<b>Effect on the study</b><br/>transport 40.1 % → 19.5 % of the<br/>supply-chain climate footprint<br/>Danish sea-transport node 857 → 53 kt<br/>Danish national footprint 86.3 → 77.2 Mt"]
...
### Auditing the correction: reproducibility, decomposition, and what the paper should now say
...
It is worth **1.11 percentage points**: transport is 15.58 % of the 2022 total without it and 16.69 % with it.
...
|:---|:---|:---|:---|
| 2019, uncorrected | 47.43 % | 47.48 % | **+0.05 pp** |
| 2022, corrected | 15.58 % | 15.65 % | **+0.07 pp** |
...
**Yes, to one percentage point on the submission's own valuation — but on EXIOBASE
v3.8.2's 2016 table, not on the v3.7 the submission cites.** Until September 2026 this document
...
| Run | Folder | Transport share of the total, submission's valuation | The same, margins at the trade industries |
|:---|:---|:---|:---|
| Manuscript, as reported | — | 46 % | — |
| Ours, v3.8.2 `IOT_2016`, uncorrected | `2019_uncorrected/` | **47.0 %** | 47.4 % |
| Ours, v3.7 `IOT_2016`, uncorrected (variant a) | `2019a/` | **35.0 %** | 36.8 % |

The submission's valuation mapped the whole of pharmaceutical and appliance
spending to the manufactured good, distribution margins included; the reproduction
is judged on that column, and the second column is what the same runs give with the
margins at the trade industries and commuting estimated directly (14 September
2026). The first column is from the runs of 12 September 2026 and has not been
re-run on the direct commuting estimate, which adds to every total and would lower
both of its shares. On the purchased-product basis the same pair reads 40.9 % and
25.9 % on the submission's valuation (41.5 % and 27.6 % in the current runs), so
the gap is not an artefact of which marginal is quoted.

The 1.0 pp residual against v3.8.2 is the demand-vector difference (finding F1 in
...
euro of Danish health expenditure — 8,152.69 kt against 6,309.86 kt in total,
with transport almost unchanged in absolute terms (2,998.4 against 2,992.6 kt)
...
to move the share by twelve points. Either way the finding is reproducible from
*a* release of the data available, and neither reading makes it an analytical
error. The measurement is published, for all thirteen configurations and all five
indicators, in
...
|:---|:---|:---|
| 2019, 2016 background, uncorrected | **55.2 %** | n/a |
| 2022 demand and v3.8.2 background, still uncorrected | 40.1 % | -15.1 pp |
| **Danish sea-transport reallocation applied** | 19.5 % | **-20.6 pp** |
| Bottom-up items included in the denominator | **15.6 %** | -3.9 pp |
...
2019 data, then the year — the steps are -27.8 pp and -7.9 pp
(`06_benchmarks_validation/year_comparison_two_step_bridge.csv`). **In either order
the reallocation is the larger step**: 20.6 against 15.1 points when the year moves
first, and 27.8 against 7.9 points when the correction is applied first.
...
will both ask: how can a share fall from 46 % to 16 % and the study still be the same
study?
...
> On the corrected model it is 19.5 % of the 3,415 kt multi-regional supply-chain
> component and 15.6 % of the 4,267 kt total that additionally carries the bottom-up
> Danish items, measured on the producing-node perspective in both cases. Re-running the
> present pipeline on the submitted reference year and correction state, on EXIOBASE
> v3.8.2's 2016 table, returned 47.0 % of the total on the submission's own valuation,
> against the manuscript's 46 %; on the v3.7 release the manuscript cites the same run
> returned 35.0 %, so the submitted figure is reproducible on one release of the
> background and not on the other. With distribution margins placed at the Danish trade
> industries, as the basic-price table records them, the re-run gives 55.2 % of its
> supply-chain component. The fall from there to 19.5 % has two sources, and a third
> step changes only what the share is a share of.
>
> The first source is the reference year, the background release and the demand vector
> moving together, and it accounts for 15.1 points. The submitted figure was computed for
> 2019 Danish health expenditure on a 2016 EXIOBASE background; the present analysis uses
> 2022 expenditure on the 2022 table of the same release family, so that expenditure year
> and model year coincide and no deflation is required. Moving to 2022 expenditure and the
> 2022 background, with no other change, brings the share to 40.1 %: Danish health
> expenditure grew faster than its transport content, and the health-care footprint as a
> whole falls from 6,310 kt to 5,688 kt across the two uncorrected configurations.
>
> The second source is the correction to the Danish sea-transport allocation described in
> the methods, and it accounts for 20.6 points, more than the year, the release and the
> demand vector combined. Rescaling one row of the Danish block from EXIOBASE's 73.65 %
> domestic-intermediate share to the 6.51 % that Statistics Denmark's own domestic
...
> falls from 857 kt to 53 kt of the health-care footprint, a reduction of 94 %, because the
...
>
> The third step is the denominator, and it accounts for the remaining 3.9 points.
> Including the bottom-up Danish items — anaesthetic gases, inhaler propellants, the direct
> fuel and waste accounts, commuting and patient and visitor travel — adds 852 kt of
> entirely domestic emissions to the total, which dilutes every supply-chain share
> proportionately. This step changes no emission estimate; it changes what the share is a
> share of, which is why the basis is stated wherever a transport figure appears.
>
> What survives the correction is substantial and should not be understated. Transport
> remains the largest producing-sector group of the input-output model at 665 kt, second
> in the footprint only to private travel (708 kt), and sea and coastal water transport
> across all regions remains 444 kt, or 10.4 % of the total —
> genuine international shipping in Danish health supply chains, now carried
> overwhelmingly by foreign rather than Danish operators, since no Danish source licenses a
> correction to another country's block. What changes is the headline: measured by
> purchased product, the chemical and pharmaceutical group is the largest contributor at
> 1,154 kt, or 27.1 %, against transport's 567 kt, or 13.3 %. The claim that transport
> dominates the Danish health-care footprint does not survive; the claim that it is one of
> its largest components does.
...
the intermediate figure above is now published at `2022_uncorrected/` (40.1 %)
rather than existing only as a documented, unpublished figure; measured from it
directly, the climate footprint is 5,688.1 kt CO2eq and transport is 34.1 % of
the total (40.1 % of the MRIO supply-chain component alone). The `2019c/`
...
moves transport from 47.4 % of the total to 21.3 % (27.4 % of the MRIO
...
share from 21.3 % to 15.6 %. `analysis.year_comparison.two_step_bridge`
...
Transport is **still the largest producing-sector group of the input-output model** at
15.6 % of the total climate footprint, second only to private travel (16.6 %) once the
bottom-up terms are counted, and the fourth largest purchased-product group at 13.3 %;
**sea and coastal water transport alone is 10.4 %**, the single largest transport component even after
correction, reflecting genuine international shipping in
Danish health supply chains.

What changes is the ranking. **Pharmaceuticals and chemical products is the largest
contributor at 27.1 %**, not transport. The paper's central claim has to move accordingly,
...
- **The 2022 target is lower than the 2019 one, and a reader should know why.**
  ...
  0.9 % of the 2022 total, against applying the published 0.09.
...
- **The `TAUX` classification question** is inherited from the
  ...
  transport share by a further 1.11 percentage points.
...
[above](#the-corrected-transport-share)). v3.10.2 was withdrawn: its 2022
...
- **Expenditure, basic prices:** built from the published 117-industry IO workbook
  ...
  at 7.4396 DKK/EUR (DNB 2022 average). The trade rows (NACE 45, 46, 47) of the two goods
  columns are the distribution margins, €790.7 M (40.5 %) of pharmaceuticals and €595.8 M
  (54.4 %) of appliances; they enter $y_H$ at the Danish motor-trade, wholesale and retail
  nodes, and only the remainder is distributed over the regions supplying the good. Cross-check: SHA1 CHE 2022 = 271.9 bn + social
...
- **Direct emissions:** DRIVHUS 2022, QA 92 + 870000 19 + α × 880000 60 − medical
  N₂O 10.07 = **119.5 kt CO₂e**. The medical N₂O is netted like for like: the 38 t of
  NID category 2.G.3.a on the accounts' own AR5 factor, 38 t × 265 = 10.07 kt, rather
  than the accounts' rounded 11 kt for the whole hospital industry; the bottom-up
  anaesthetic term adds the same 38 t back on AR6. The eldercare share α = **0.3092**
  is read from the analysis year's own IO table (industry 880000's deliveries to
  eldercare 13302 vs childcare 13301: 15.54 vs 34.72 bn DKK), replacing the 0.4914
  carried forward from the 2019 detailed SUT; the 2016 and 2019 runs read 0.3455 and
  0.3060 from their own tables, and the same α weights the direct waste and the
  commuting hours share. The care boundary sets the weights on 870000 and 880000:
  (0, 0) for health only, (1, α) for the default, (1, 1) for zorg en welzijn.
- **Direct waste:** AFFALD01 2022 (excl. soil), same boundary and same α = **42.8 kt**.
- **Bottom-up:** anaesthetics 11.7 kt (N₂O 10.4 from NID 2.G.3.a, volatiles 1.4
  from medstat.dk ATC N01AB sales on AR6 factors, no longer a proxy; see
  ["Bottom-up anaesthetic gases"](#bottom-up-anaesthetic-gases) below);
  pMDI **12.6 kt** (Danish EPA F-gas inventory 2022, restated from the inventory's AR4
  factors to AR6); employee commuting **444.2 kt**, estimated directly as
  $G_c = \delta_w \times 365 \times P_{6+} \times s_h \times \eta_c$, with $\delta_w$ the
  TU workplace distance of 9.3 km per person per day (both legs), $P_{6+}$ the
  5,499,115 residents aged 6 and over, $s_h$ health care's share of hours worked,
  0.12780 (NABB117: $(860010 + 860020 + 870000 + \alpha \times 880000)$ over all
  industries), and $\eta_c$ the Dutch commuting inventory's life-cycle intensity,
  573.43 kt over 3,080 million person-km = 0.18618 kg CO₂e per person-km; patient and
  visitor travel 263.6 kt from the TU survey (0.8 km per person per day for purpose 33).
  Neither travel term is a scaling factor any longer. The commuting ratio it replaced
  carried the Dutch base's count of one trip per working day into a survey
  that counts both legs.
...
["The corrected transport share"](#the-corrected-transport-share) above
...
|:---|:---|:---|:---|:---|
| Climate change | **4,266.9 kt CO₂e** (726 kg per person) | 77,240.6 kt | 5.5 % | 4.4 % |
| Material extraction | 3,498.3 kt | 53,925.1 kt | 6.5 % | 6.4 % |
| Blue water | 74.8 Mm³ | 1,276.1 Mm³ | 5.9 % | 5.8 % |
| Land use | 4,117.7 km² | 99,442.9 km² | 4.1 % | 4.1 % |
| Waste generation | 221.1 kt | 10,586.4 kt | 2.1 % | 1.7 % |
...
**S1 131.2 / S2 73.3 / S3 3,796.9 / outside-protocol 263.6 kt CO₂e**, with the 1.8 kt
self-supply loop removed.
...
**By producing sector group (climate, share of the 4,267 kt total):**
private travel 16.6 %, transport 15.6 %, food and catering 12.4 %, coal and
petroleum 11.2 %, electricity 8.4 %, chemicals 7.5 %, services 6.3 %, steam and
hot water 5.5 %, waste management 4.8 %.
...
**4,289 kt**, 95 % interval **3,675 to 5,029 kt**, CV **8.00 %**, alongside
Lenzen et al.'s published 8.35 % for Denmark. First-order variance shares: MRIO
parameters 69.6 %, the covariance between commuting and visitor travel 13.4 %,
commuting 9.0 %, visitor travel 7.9 %; every other bottom-up item below 0.2 %.
...
Steenmeijer, Eckelman, Lenzen, and Pichler. Including it adds **15.1 %**
(exogenous CFC from Danish national accounts) or **21.2 %** (endogenised on the
published Södersten et al. 2018 capital matrices; our own simplified
endogenisation gave 23.0 %, which the published route reproduces to within 1.7
percentage points), each on the 3,535 kt input-output component and direct row. See ["Capital (GFCF) treatment"](#capital-gfcf-treatment) below.

Scope 2 is 1.8 % of the total, against Arup's 8.3 % for Denmark in 2014. The
...
not. Revision sensitivity, healthcare supply chain: SAR 3,254 · TAR 3,294 ·
AR4 3,347 · AR5 3,425 · **AR6 3,415 kt**. **3.7 % (126 kt) cannot be restated at
...
| Item | Value | Source |
|:---|:---|:---|
| Anaesthetic gases | **11.7 kt** (N₂O 10.4 + volatiles 1.4) | medstat.dk (Danish Health Data Authority) ATC N01AB sales (sevoflurane 2,400 L, desflurane 181 L, isoflurane 15 L), specific gravities from Laster et al. 1994, GWP₁₀₀ from IPCC AR6 table 7.SM.7 (sevoflurane 195, desflurane 2,590, isoflurane 539), 5 % of sevoflurane metabolised (Kharasch et al. 1995); N₂O from NID 2.G.3.a × 273 |
...
| Employee commuting | **444.2 kt** | TU (DTU) Tabel 15, workplace purpose, 9.3 km/person/day counting both legs; residents aged 6+; health care's share of hours worked, NABB117; Dutch inventory intensity 0.18618 kg CO₂e per person-km (RIVM 2022-0159) |
| pMDI propellants | 12.6 kt | Danish EPA F-gas inventory 2022, HFC-134a and HFC-227ea restated from AR4 (1,430; 3,220) to AR6 (1,530; 3,600); 2019 from the same inventory (Environmental Project No. 2161: 5.0 t HFC-134a, 0.7 t HFC-227ea), 10.17 kt |
| Direct operational | 119.5 kt CO₂e, 42.8 kt waste | DRIVHUS, AFFALD01; medical N₂O netted at 38 t × 265 = 10.07 kt |
...
England → Netherlands → Denmark double transplant. Commuting no longer uses the
ratio either: since 14 September 2026 it is built the same way, from the survey's
workplace distance and health care's share of hours worked, and its four non-climate
indicators scale by the activity ratio to the Dutch item (0.7746 for 2022). The anaesthetics item is no
...
|:---|:---|:---|:---|:---|:---|:---|:---|:---|
| health only (no eldercare) | 31,079 | 3,462 | 4.48 % | 0.589 | 2,915 | 63.7 | 3,360 | 178.2 |
| **health + eldercare (default)** | **40,597** | **4,267** | **5.52 %** | **0.726** | **3,498** | **74.8** | **4,118** | **221.1** |
| + childcare ("zorg en welzijn", Steenmeijer-comparable) | 49,709 | 4,994 | 6.47 % | 0.850 | 4,055 | 85.4 | 4,843 | 259.3 |
...
mainstream, and the three boundaries span −19 % to +17 % around it on climate. Since
14 September 2026 the direct emissions, the direct waste and the commuting hours
follow the boundary too: the health-only row carries no residential care or social
work operations, and the childcare row carries all of industry 880000.
...
4.0 Mt CO₂ (CO₂-only), 6.4%. Our 4.27 Mt / 5.5 % / 0.73 t per capita sits coherently
...
Closed since this list was first written: the volatile anaesthetics are read from the
medstat.dk register, patient travel from the TU survey, and the eldercare share α from
the analysis year's own input-output table. Still open:
...
the visitor-to-patient ratio, imported from NHS England because TU does not separate
hospital visits; distribution margins charged at the intensity of the Danish trade
industries (a whole-column mapping would add 11.5 % to the climate footprint);
capital excluded (Steenmeijer-consistent); pharma mapping as Scenario B; EXIOBASE
recipe biases quantified in `recipe_validation_2022.csv` → motivate the Danish-SNAC
phase (see
...
This bridge predates the corrections of 13 and 14 September 2026 and is kept as the
record of that reconciliation, not as a current result: its step 7 carries commuting on
the ratio scaling since retired, and its step 2 the eldercare share of the 2019 detailed
supply-use table. The current 2019 run on the corrected background is variant `2019c`,
3,995.6 kt CO₂e, with commuting at 429.3 kt estimated directly and a direct operational
term of 139.4 kt.
...
|:---|:---|:---|:---|
| Climate change (kt CO₂e) | **3,533.9** | 4,068.6 (**+15.1 %**) | 4,346.2 (**+23.0 %**) |
| Material extraction (kt) | **3,466.3** | 4,260.2 (**+22.9 %**) | 4,718.9 (**+36.1 %**) |
| Blue water (Mm³) | **74.6** | 81.6 (**+9.5 %**) | 84.2 (**+12.9 %**) |
| Land use (km²) | **4,115.6** | 4,623.8 (**+12.3 %**) | 4,991.3 (**+21.3 %**) |
| Waste generation (kt) | **221.1** | 244.8 (**+10.7 %**) | 260.7 (**+17.9 %**) |
...
|:---|:---|:---|:---|
| Climate change (kt CO₂e) | 3,533.9 | **4,284.9** | **+21.2 %** |
| Material extraction (kt) | 3,466.3 | 4,814.8 | +38.9 % |
| Blue water (Mm³) | 74.6 | 83.8 | +12.5 % |
| Land use (km²) | 4,115.6 | 5,043.2 | +22.5 % |
| Waste generation (kt) | 221.1 | 264.0 | +19.4 % |

This result **validates the simplified construction** reported above, which gave
+23.0 % on climate against the published method's +21.2 %. The two agree to
1.7 percentage points, so the simplified version was adequate for the magnitude
...
misallocation documented in [the corrected transport share](#the-corrected-transport-share) above.
...
How far the evidence on volatile anaesthetic gases goes, and where the 11.7 kt
anaesthetic-gas item used in the [2022 headline](#the-2022-primary-analysis) above comes from.

### Where the number comes from now

Our anaesthetic-gas item (11.7 kt CO₂e) has two parts:
...
| **volatile agents** (sevoflurane, desflurane, isoflurane) | 1.4 kt | medstat.dk register, ATC N01AB, actual Danish sales for the year: sevoflurane 2,400 L, desflurane 181 L, isoflurane 15 L; specific gravities from Laster et al. (1994); GWP₁₀₀ from IPCC AR6 (Smith et al. 2021, table 7.SM.7: sevoflurane 195, desflurane 2,590, isoflurane 539), with 5 % of sevoflurane treated as metabolised. Sulbaek Andersen et al. (2023) factors, used until 13 September 2026, gave 1.2 kt | **strong**: a Danish measurement, not a transfer |
...
The volatile component is now **1.4 kt**, computed from the Danish sales
register rather than transferred from the Netherlands. Each agent's volume is
converted to mass with its published specific gravity and characterised with its
own AR6 GWP₁₀₀, so the figure carries no cross-country scaling assumption at all, and
...
(5.87/17.28). It is recorded here because the two agree to within 4 per cent,
...
**Materiality:** the entire anaesthetic item is 11.7 kt of a 4,267 kt footprint
(0.28 %), and its exact first-order variance share is **0.009 %**. Even a
...
inside the accounts. Its medical part, the 38 t per year of NID category 2.G.3.a, is
netted out of the Scope 1 figure on the accounts' own AR5 factor, 38 t × 265 =
10.07 kt, and the bottom-up anaesthetic item then adds the same 38 t back on AR6
(10.37 kt). Until 14 September 2026 the subtraction was the accounts' rounded 11 kt,
a different quantity on a different basis from the term added back.

### Recommendation

Done: the medstat ATC N01AB series is read for each analysis year and the Danish
estimate is computed directly, citing Talbot et al. (2025) for the method and IPCC
AR6 for the factors. Report the materiality above beside it.
...
The shipping row is the one [this study reallocates](#the-corrected-transport-share),
...
Our 2022 value is the Danish EPA F-gas inventory's actual reported MDI emission (a national
measurement, not a scaled proxy): 11.6 kt on the inventory's own AR4 GWP100 factors, and
12.6 kt restated to the AR6 factors the model uses. Both sit within about 15 % of the GWP100
conversion of Vestbo.
...
Our 2022 figure is 1.38 kt CO₂e on the IPCC AR6 GWP100 factors (Smith et al. 2021, table
7.SM.7) with 5 % of sevoflurane treated as metabolised; the Sulbaek Andersen et al. (2023)
factors used until 13 September 2026 gave 1.20 kt. The item is small, but it is a named omission rather than an
uncertainty.
...
data, but the derivation now matches the appendix. Since 14 September 2026 commuting
does not use the ratio at all: it is estimated directly from the survey's workplace
distance and health care's share of hours worked, because the Dutch base the ratio
scaled counts one commuting trip per working day while the Danish survey counts both
legs.
...
Correcting it takes transport from 40.1 % to 19.5 % of the supply-chain footprint. See
["The corrected transport share"](#the-corrected-transport-share) above for the full
audit.
...
| Climate change | 4,815 kt (5.6 %) | 4,267 kt (5.5 %) |
| Material extraction | 2,601 kt (5.5 %) | 3,498 kt (6.5 %) |
| Blue water | 47 Mm³ (4.3 %) | 74.8 Mm³ (5.9 %) |
| Land use | 2,753 km² (3.6 %) | 4,118 km² (4.1 %) |
| Waste generation | 840 kt (3.6 %) | 221 kt (2.1 %) |

Climate falls by 11 % although the demand vector is 57 % larger, because two corrections
outweigh the growth: the removal of the phantom shipping emissions, and the
distribution margins on medicines and appliances, which are no longer charged at
manufacturing intensity. The other three resource categories rise by a third to three
fifths, less than the demand vector, for the second of those reasons. Waste falls because the 2011 hybrid waste extension was replaced with
...
**The decisive test has since been run.** The corrected pipeline on 2019 expenditure and
the 2016 background (variant `2019c`, with year-aware output routing so that it cannot
overwrite the 2022 headline) gives 3,996 kt CO₂e. Of the 548 kt between the submitted
4,815 kt and the 2022 headline, the method change on the submitted reference year accounts
for −819 kt and the move to 2022 for +271 kt, the latter in current prices.
