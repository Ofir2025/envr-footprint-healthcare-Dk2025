...
currency conversion c (their §2.2, pp. 636–637). Same maths our pipeline already uses;
the innovation is entirely in the data plumbing.
...
1. **The input-recipe problem.** Today the entire healthcare-services footprint hangs on
   ...
   care vs social work) from the national SUT. **This is the decisive scientific gain.**
...
5. **Refinement shortcut (low-cost first iteration):** before the full SNAC, apply the
   **Piñero et al. (2018) correction-matrix bridge** (C = P p̂_A⁻¹; W = Cα̂; R* = W∘M) to
   ...
   rebuilding anything. This alone addresses weakness 3 and much of 2.
6. **Waste:** with the national core in place, the waste extension becomes a **WIO-style
   ...
   of the world. This replaces the 2011-absolute-tonnes carry-forward entirely.
...
- **Phase H1 (~days):** Piñero bridge on imports + validation of the EXIOBASE DK health
  ...
  this yields the recipe-error quantification the transport headline needs *now*.
- **Phase H2 (~weeks):** full Danish-SNAC 2022 build per §4, with the WIO-style waste rows.
  This is a methods paper in its own right (Palm et al. was exactly that for Sweden) and
  the natural vehicle for the Denmark-2022 flagship analysis.
