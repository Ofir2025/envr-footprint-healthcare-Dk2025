...
*Note.* The formal response letter submitted to the journal is the Word document
`docs/eriksen_et_al_2026/eriksen_et_al_2026_reviewer_responses.docx`, kept off
the public branch because it quotes the referee reports. This file is its
technical companion and carries the repository detail behind each answer. Both
carry the results of 14 September 2026.
...
required us to **correct one of the submitted paper's findings**. We set that
...
which covers every published file, as the consistency audit checks).
...
| Climate footprint | 4,815 kt CO₂e, 5.6 % of the national total | **4,267 kt CO₂e (95 % uncertainty interval 3,675 to 5,029 kt), 5.5 %** |
| Range across the five categories | 3.6 % to 5.6 % of national | **2.1 % to 6.5 %** |
| Leading contributor | transport | **pharmaceuticals and chemical products, 27.1 % of the climate footprint and the largest in four of the five categories** (food leads land use) |
| Valuation of medicines and appliances | whole category mapped to the manufactured good | **distribution margins placed at the Danish trade industries**, as the basic-price table records them |
...
| Transport finding | largest contributor: 39 % of the input-output component by purchase, 46 % by producing sector | **corrected** to 13.3 % and 15.6 %: a documented EXIOBASE misallocation |
...
|:---|---:|---:|---:|---:|---:|---:|
| Pharmaceuticals and chemical products | 1,154.3 | 1,158.0 ± 96.0 | 8.3 % | 982 to 1,358 | 27.1 % | 24.2 to 29.0 % |
| Services | 791.6 | 794.1 ± 66.6 | 8.4 % | 672 to 933 | 18.6 % | 16.6 to 19.9 % |
| Individual travel | 707.7 | 733.7 ± 189.1 | **25.8 %** | 434 to 1,169 | 16.6 % | **10.7 to 25.2 %** |
| Transport | 566.5 | 568.3 ± 47.6 | 8.4 % | 481 to 668 | 13.3 % | 11.9 to 14.3 % |
| Food and food services | 433.7 | 435.1 ± 36.5 | 8.4 % | 368 to 511 | 10.2 % | 9.1 to 10.9 % |
| Other | 293.0 | 293.9 ± 24.6 | 8.4 % | 249 to 345 | 6.9 % | 6.1 to 7.4 % |
| Operational impacts | 131.2 | 132.2 ± 12.0 | 9.1 % | 110 to 157 | 3.1 % | 2.4 to 3.9 % |
| Medical, electrical equipment and machinery | 114.6 | 114.9 ± 9.6 | 8.4 % | 97 to 135 | 2.7 % | 2.4 to 2.9 % |
| Heat and electricity | 74.2 | 74.4 ± 6.2 | 8.4 % | 63 to 87 | 1.7 % | 1.6 to 1.9 % |
...
exceptions are the finding. Individual travel carries **25.8 %**, three times the
...
ratio.** Pharmaceuticals move by −15 % to +18 % in level and by 24.2 % to 29.0 %
in share. Reporting only the level interval would overstate the uncertainty on
every statement of the form "pharmaceuticals are 27 % of the footprint", which is the
form most of the paper's claims take.

**The ranking is robust at the top and not at the middle.** Pharmaceuticals lead
in 96.6 % of draws. Individual travel's share spans 10.7 % to 25.2 %, which overlaps
Services, Transport and Food and reaches the lower end of the pharmaceutical
interval (travel leads in the remaining 3.4 % of draws), so its rank is genuinely
uncertain - `uncertainty_ranking_probabilities.csv` gives the probability of each
group holding each of the first three places.
...
1. **Every multiplier has median 1**, so the analysis adds dispersion without
   ...
   deterministic model to within 1 % (+0.52 %, or +22 kt on 4,267 kt) rather
   than exactly. A distribution whose median is not 1 would shift the central
   estimate silently; this one does not.
2. **Structural choices are sensitivity cases, never distributions.** Price
   base year, waste reference year, and the pharmaceutical mapping are reported as
   sensitivity cases; burying a structural choice inside a lognormal would misrepresent
...
3. **Input-output parameter uncertainty is included**, as a shared factor
   ...
   **transfer**, not a reproduction: Lenzen et al. obtain 8.35 % in Eora for a
   ...
   4.267 Mt, 50 % larger. Their own supplementary analysis makes the relative
...
|:---|:---|:---|
| ρ = 1.00, perfect (study default) | **7.98 %** | 3,680 to 5,021 |
...
variation collapses to 3.91 % against the 8.35 % asserted. We re-solve the
...
three totals differ by 0.15 percentage points, while the median coefficient of
variation of a single contribution group runs from 8.4 % to 17.6 %. Our default
...
at seed 42, so the default row reads 7.98 % where the headline reads 8.00 %.
...
six parameters the closed-form inflation is **+0.93 %** (the input-output factor
accounts for +0.28 pp of it, patient and visitor travel for +0.36 pp and employee
commuting for +0.26 pp); the simulated mean sits +0.89 % above the deterministic
estimate. Both
mean and median are reported.

*First-order Sobol indices exclude interaction variance.* In this additive model
the six own-variance terms sum to **86.6 %**. The remaining **13.4 %** is not
...
**Denmark 2022, climate change: median 4,289 kt CO₂-e, 95 % interval
3,675 to 5,029 kt, coefficient of variation 8.00 %** (Tier 2 simulation; the
IPCC Tier 1 error propagation the same standard requires alongside it gives
8.04 %), closely consistent with Lenzen's published 8.35 % for the same country.

**Exact first-order Sobol variance shares**: input-output parameters **69.6 %**,
employee commuting 9.0 %, patient and visitor travel 7.9 %, the covariance of
those two 13.4 %, and **every other bottom-up item below 0.2 %** (the largest,
direct operations, is 0.112 %). Read as a block, travel accounts for 30.3 %. This
...
- *Cross-country scaling for pMDI*: replaced by the Danish EPA F-gas inventory,
  restated from its AR4 factors to the AR6 values the model uses.
- *Cross-country scaling for private travel*: replaced by the Danish national
  travel survey (Transportvaneundersøgelsen, table 15: purpose code 33 for
  patient travel, workplace distance for employee commuting). The
...
waste-account reference year are run as **sensitivity cases** and reported
...
at one of them. The sensitivity cases also carry the most important finding of this
section: the alternative pharmaceutical mapping moves the median to 3,547 kt,
which lies entirely outside the parametric 95 % interval of 3,675 to 5,029 kt. A
...
the health sector's own self-supply loop, $s_h(L_{hh}-1)E_H$ = 1.81 kt, was
being counted both in the MRIO chain and in the national-accounts scope 1
figure, and is now subtracted. The ledger is
`02_scopes_wood_hertwich/2022c/double_counting_ledger.csv`;
its first row states
the quantity being partitioned, the MRIO footprint decomposition, at 3,415.37 kt
CO₂-eq, the same number `00_core_footprint` publishes, and its second row is the
1.81 kt loop. Medical nitrous oxide is netted out of the direct-emissions
figure on the accounts' own AR5 basis (38 t at a GWP of 265, 10.07 kt) before
the bottom-up anaesthetics item is added.
...
|:---|:---|:---|
| Excluded (baseline, as submitted) | 3,534.9 kt | n/a |
| Exogenous capital service flow | 4,069.5 kt | **+15.1 %** |
| Endogenised on the published Södersten et al. (2018) matrices | 4,285.8 kt | **+21.2 %** |
| Endogenised on our own simplified construction | 4,347.1 kt | +23.0 % |
...
first, and the published matrices then reproduced it to within 1.7 percentage
...
of why their Australian share (7 %) exceeds most others, a point we now make
...
**Danish health care's climate footprint is 31.4 % domestic and 68.6 % imported
in origin**, and the imported share of the input-output component rises with
supply-chain depth: 27 % of production layer 0 is imported, and 89 % of layer 3. The Discussion now reports the
...
- **Climate Act (Act No. 965 of 26 June 2020), §1(3)(4)** requires that Danish
  ...
  **68.6 % imported in origin** is precisely the case that provision
  contemplates.
...
Two mappings are carried as explicit sensitivity cases:

- **Case A**: pharmaceuticals mapped to EXIOBASE *Chemicals n.e.c.*, as
  submitted and as in Steenmeijer et al.
- **Case B**: a pharmaceutical-specific intensity.

| | Climate median | 95 % interval | CV |
|:---|:---|:---|:---|
| A, chemicals n.e.c. | 4,289 kt | 3,675 to 5,029 | 8.00 % |
| B, pharma-specific | 3,547 kt | 2,974 to 4,304 | 9.50 % |

The consequence is not merely a range. Under Case A, pharmaceuticals and
chemicals hold rank 1 among contribution groups in **96.6 %** of Monte Carlo draws.
Under Case B, services take rank 1 with probability 0.63, individual travel
with 0.34, and pharmaceuticals fall to 0.03. **The identity of the largest contributor
...
**Done, and the closest comparator comes within 1 % of the model's total once
...
model. A headline-to-headline comparison against our 4.27 Mt would be
...
| This study, headline (health + eldercare, capital excluded) | 4.27 | 0.726 | 5.5 % |
| + their sector boundary (NACE Q, including childcare) | 4.99 | 0.850 | 6.5 % |
| **+ their capital treatment (endogenised)** | **6.14** | **1.046** | **8.0 %** |

Boundary-matched, our total is **0.7 % above** theirs and our per-capita figure
2.3 % below, with a national share of 7.95 % against their 8.3 %. Most of the
apparent gap is the sector boundary and the capital treatment; the remainder is of
the size that the different model construction and reference year (2016 against
2022) could produce, and we do not claim more agreement than that.
...
**R1-9 requires us to correct a finding.** See "Data integrity" below. The
...
`01_eriksen_replication/2019_uncorrected/`, now with the distribution margins
placed at the trade industries: a grand total of 6,309.86 kt CO₂-eq, of which
5,424.79 kt is the MRIO supply-chain component and 885.06 kt the Danish direct
and bottom-up items (`scopes_summary.csv` in that folder). Applying the
2019 → 2016 deflator of 0.966 used in the submitted-era sensitivity takes that
component to 5,240.35 kt and the grand total to **6,125.41 kt**, i.e. the
omitted deflation overstated the figure by **184.44 kt, 2.9 %**. That is smaller than the
...
**Done, and all four now rest on Danish activity data rather than scaled Dutch values.**

| Item | Submitted | Revised | Source |
|:---|:---|:---|:---|
| Anaesthetic gases | Dutch value scaled by birth rate | **11.75 kt** = nitrous oxide 10.37 + volatile agents 1.38 | Nitrous oxide: National Inventory Document 2024 (DCE 622), category 2.G.3.a, 38 t N₂O/yr × 273 (AR6 GWP₁₀₀, the factor the model's climate row uses). Volatile agents: medstat.dk register, ATC N01AB, actual Danish sales — sevoflurane 2,400 L, desflurane 181 L, isoflurane 15 L; specific gravities from Laster et al. (1994); GWP₁₀₀ from IPCC AR6 (Smith et al., 2021, table 7.SM.7), with 5 % of sevoflurane treated as metabolised rather than exhaled |
...
| pMDI propellants | Dutch value scaled | **12.55 kt** | Danish EPA F-gas inventory, HFC-134a and HFC-227ea restated from the inventory's AR4 factors to AR6; the 2019 run uses the Danish EPA 2019 inventory (5.0 t HFC-134a, 0.7 t HFC-227ea), 10.17 kt |
| Employee commuting | Dutch value scaled | **444.2 kt** | Direct estimate from the Danish National Travel Survey (Transportvaneundersøgelsen, table 15) workplace distance, hours worked by industry (Statistics Denmark NABB117) and the Dutch commuting inventory's intensity per person-km |

**Employee commuting was re-estimated on 14 September 2026.** Until then the term
was the Dutch commuting inventory scaled to Denmark by ratios of employment,
weekly hours and daily commuting distance. That construction had two faults. The
Dutch base counts one trip per working day, whereas the Danish survey counts both
legs of the journey, so the scaled term carried about half the commuting it
described; and the weekly hours on both sides of the ratio were all-industry
values rather than health-care ones. The term is now estimated directly from
Danish data, on the same construction as patient and visitor travel:
$G_c = \delta_w \times 365 \times P_{6+} \times s_h \times \eta_c$, where
$\delta_w$ is the survey's workplace distance, 9.3 km per person per day in 2022
over both legs; $P_{6+}$ the 5,499,115 residents aged 6 and over; $s_h$ health
care's share of hours worked, 0.1278, from Statistics Denmark's hours worked by
industry (NABB117), with the eldercare share of social work, $\alpha = 0.3092$,
read from each year's input-output table; and $\eta_c$ the Dutch commuting
inventory's life-cycle intensity, 573.43 kt over 3,080 million person-km, or
0.1862 kg CO₂e per person-km. The 2022 term rises by 103.8 kt, to 444.2 kt. The
patient and visitor term, 263.6 kt, is unchanged.
...
|:---|:---|:---|
| Employee commuting | 1.25 | 8.996 % |
| Patient and visitor travel | 1.40 | 7.924 % |
| Covariance of those two, which share a method (ρ = 0.8) | — | 13.363 % |
| Direct operations | 1.10 | 0.112 % |
| Anaesthetic gases | 1.30 | 0.0090 % |
| pMDI propellants | 1.15 | 0.0027 % |

Read as a block, travel accounts for 30.3 % of the variance; the remaining
bottom-up items account for 0.12 % between them, and the input-output model for
69.6 %. Commuting and patient travel are the only two of the four that matter,
and they matter jointly, because both are built on the same travel-survey table,
which is why we model their correlation rather than assume it away, and report the result
at ρ ∈ {0, 0.5, 0.8} (`uncertainty_travel_correlation.csv`). Treating them as
independent would narrow the 95 % interval by about 43 kt at the lower end and
55 kt at the upper.
...
**Accepted: the revised paper keeps the two apart.** It is diagnostic. It
identifies where the impacts associated with Danish health care arise, by
purchase category, producing sector and region, and it does not estimate the
mitigation potential of any intervention. Wording in the Results that read a
hotspot as a mitigation opportunity has been removed, the Conclusion calls
procurement, travel and food levers because a purchaser controls them rather than
because their effect has been shown, and the Limitations end:

> *[Referee comment withheld - the referee reports for a manuscript under
> review are confidential. The full text is in the submission system and in
> the private working copy. The heading above states the point addressed.]*

The multi-impact result is also the argument for why that modelling must cover all
five categories: the largest sources differ by pressure (Figure 1), so a measure
designed against climate change alone can shift the burden to water, land or
waste.

**Held back for the follow-up paper (14 September 2026).** A scenario layer was
built during the revision. It is not in the revised manuscript, Appendix A or either
response letter, because the authors decided to publish the mitigation analysis as
its own paper. Its results, code and figures are kept in the working copy for that
paper and withheld from the published branch (`analysis.gold_scope` classifies the
layer as withheld); nothing in the revision cites them.
...
pharmaceuticals and chemical products, the single largest lever at 27.1% of the
climate footprint; employee commuting and patient and visitor travel, 16.6% and
the part of the system a health authority controls most directly through siting,
scheduling and transport provision; and food procurement, 10.2% of the climate
footprint and 38% of land use.
...
published matrices raises the input-output component from 3,535 to 4,286 kt, so the
omission is worth about 21%), and states
which parameter would repay further work: individual travel is the only line
whose coefficient of variation, 25.8%, is materially above the 8.3-9.1% that the
...
because more than two-thirds of the variance is multi-regional input-output
parameters.
...
Addressing R1-1 and R2-10 required re-examining the background data and the
expenditure vector. Five findings emerged that affect the submitted results. We report them because they
are material.

### 1. EXIOBASE's allocation of Danish sea transport inflated the transport share

The submitted manuscript reported transport services as the largest contributor, at 39 % of
the input-output component, and the transport sector at 46 % of emissions by producing sector. On a sound model that result reproduces (40.1 % of the supply-chain component, `2022_uncorrected/`), with Danish sea and
coastal water transport alone contributing 857 kt.
...
|:---|:---|:---|
| Transport share of the supply-chain climate footprint | 40.1 % | **19.5 %** |
| Danish sea transport as a producing node | 857 kt | **53.3 kt** |

The 19.5 % is on the 3,415 kt supply-chain component; on the 4,267 kt total,
which additionally carries the entirely Danish bottom-up items, transport is
15.6 %. The revised manuscript reports transport at 15.6 % of the footprint and explains why the
...
at 46 % of sector contributions; with the submission's own treatment of
distribution margins, a genuine v3.7 run returns **35.0 %**, while v3.8.2's 2016
table without the shipping correction returns **47.0 %** (both on the runs of 12
September 2026, before commuting was re-estimated). And v3.7
...
**IPCC AR6** rather than the AR4 factors the workbook actually contained. The
bottom-up gases are on the same basis: the inhaler propellant inventory, which
reports on AR4, is restated to AR6, and the volatile anaesthetics take the AR6
factors.

### 5. Distribution margins had been counted as manufactured goods

Statistics Denmark's input-output table records final demand at basic prices,
which exclude product taxes but record the wholesale and retail margin on a good
as a purchase from the trade industry that earned it. The submitted analysis
mapped the whole of pharmaceutical expenditure to *Chemicals n.e.c.* and the
whole of appliance expenditure to *Medical, precision and optical instruments*,
so the margins, 40.5 % of pharmaceutical and 54.4 % of appliance expenditure in
2022, were charged at manufacturing intensities in the supplying regions. The
revision places them at the Danish motor-trade, wholesale and retail industries
and distributes only the remainder over the regions supplying the good. The
correction is worth 491 kt CO₂e in 2022: charging the margins at manufacturing
intensity would give 4,758 kt against the 4,267 kt headline, 11.5 % more, and
every other category moves by a similar or larger proportion (765 kt of material
extraction, 20.7 Mm³ of blue water, 735 km² of land use and 38 kt of waste); the revised manuscript states the valuation in the Methods and the
size of the effect in the Limitations.

---

## What we have not done

- **Mitigation potential is not estimated** (R2-7). The paper identifies where
  impacts arise; testing interventions across all five categories needs scenario
  modelling, which is left to the follow-up paper.
