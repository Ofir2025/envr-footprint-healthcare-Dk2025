...
| [18](#r18) | Withheld for a follow-up paper | | |
...
flowchart LR
    ...
    P --> G1["<b>geographical displacement</b><br/>domestic 22.7 %, imported 77.3 %"]
    Q --> G2["<b>procurement levers</b><br/>domestic 58.1 %, imported 41.9 %"]

    P --> F["<b>f = 3,415.4 kt CO<sub>2</sub>-eq</b><br/>both marginals sum to f exactly,<br/>asserted at write time"]
    Q --> F
...
- **Basic prices, with the distribution margins placed at the trade industries.**
  EXIOBASE is in basic prices, and so is the Statistics Denmark input-output table the
  expenditure is read from: its purpose columns exclude VAT and other product taxes. At
  basic prices, however, the wholesale and retail margin on a good is recorded as a
  purchase from the trade industry that earned it (rows of NACE divisions 45, 46 and 47),
  not as part of the good. In 2022 those rows are 40.5 % of pharmaceutical and 54.4 % of
  appliance expenditure. The margin is therefore entered at the Danish motor-trade,
  wholesale and retail nodes, and only the remainder is distributed over the regions
  supplying the good. Until 13 September 2026 the whole category was mapped to the good,
  which overstated the goods' supply chains by the margin share
  ([defects_and_fixes.md](../revision/defects_and_fixes.md#findings-of-13-september-2026)).
  `expenditure_summary.csv` reports the expenditure, the margin and $y_H$ per component.
...
|:---|:---|:---|:---|:---|
| `2016a/` | a | `2016_v3_7` | 7,747.44 | 36.73 % |
| `2016b/` | b | `2016_v3_7_snacship` | 5,787.75 | 17.42 % |
| `2016c/` | c | `2016_snacship` | 3,809.18 | 21.19 % |
| `2016d/` | d | `2016_snacship_capital_zorg_en_welzijn` | 5,747.61 | 19.27 % |
| `2016_uncorrected/` | - | `2016` | 6,005.24 | 47.281 % |
| `2019a/` | a | `2016_y2019_v3_7` | 8,152.69 | 36.78 % |
| `2019b/` | b | `2016_y2019_v3_7_snacship` | 6,088.38 | 17.45 % |
| `2019c/` | c | `2016_y2019_snacship` | 3,995.59 | 21.30 % |
| `2019d/` | d | `2016_y2019_snacship_capital_zorg_en_welzijn` | 6,018.05 | 19.36 % |
| `2019_uncorrected/` | - | `2016_y2019` | 6,309.86 | 47.43 % |
| `2022c/` | c | `2022_snacship` | 4,266.88 | 15.58 % |
| `2022d/` | d | `2022_snacship_capital_zorg_en_welzijn` | 6,220.59 | 14.63 % |
| `2022_uncorrected/` | - | `2022` | 5,688.08 | 34.06 % |
...
EXIOBASE v3.7's `IOT_2016_ixi` uncorrected: 7,747.44 against 8,152.69 kt, a 5.2 %
difference that is Danish health expenditure growth between 2016 and 2019 together
with each year's own Danish bottom-up terms. The transport share is 36.73 % against
36.78 %, which is the same model structure reporting the same thing twice, as it
should.
...
transport at 46 % of sector contributions; `2019_uncorrected` returns 47.43 %,
while variant `a` — the release the submission actually ran on — returns 36.78 %.
...
>
> Reading them the intuitive way inverts the domestic/imported split: 31.4 % of the
> climate footprint is domestic on the producing-node basis, and 66.5 % on the
...
| transport 36.8 % of the supply-chain footprint | 19.5 %, or 15.6 % of the total | EXIOBASE artefact, see [section 10](#r10) |
...
The spread is 72.1-75.0 kt: 4.0 % of Scope 2 and **0.07 % of the total footprint**. The
...
figure already contains. It is 1.81 kt. Subtracting it is what makes Scope 1 and
...
- The manuscript layer ([section 01](#r01)) totals 4,266.88 kt using the
  Hertwich & Wood Scope 2; this folder totals 4,265.07 kt using the strict-protocol
  Scope 2 and removing the self-supply loop. Both are correct on their stated basis, and
  audit check C1 reconciles them: 4,265.07 + 1.81 = 4,266.88. The two terms are
  rounded independently, so adding the printed figures gives 4,266.88; the check
...
|:---|:---|:---|:---|:---|
| `2016a` | v3.7 `IOT_2016_ixi` | not applied | 7,746.657937 | `01_/2016a`, 7,747.435964 |
| `2016b` | v3.7 `IOT_2016_ixi` | applied | 5,786.971118 | `01_/2016b`, 5,787.749144 |
| `2016c` | v3.8.2 `IOT_2016_ixi` | applied | 3,807.376937 | `01_/2016c`, 3,809.181659 |
| `2016d` | v3.8.2 `IOT_2016_ixi` + capital | applied | 5,745.357042 | `01_/2016d`, 5,747.613288 |
| `2019a` | v3.7 `IOT_2016_ixi` | not applied | 8,151.873291 | `01_/2019a`, 8,152.692844 |
| `2019b` | v3.7 `IOT_2016_ixi` | applied | 6,087.557086 | `01_/2019b`, 6,088.376637 |
| `2019c` | v3.8.2 `IOT_2016_ixi` | applied | 3,993.690251 | `01_/2019c`, 3,995.591845 |
| `2019d` | v3.8.2 `IOT_2016_ixi` + capital | applied | 6,015.673572 | `01_/2019d`, 6,018.045392 |
| `2022c` | v3.8.2 `IOT_2022_ixi` | applied | 4,265.068062 | `01_/2022c`, 4,266.880462 |
| `2022d` | v3.8.2 `IOT_2022_ixi` + capital | applied | 6,218.340862 | `01_/2022d`, 6,220.594483 |
| `2016_uncorrected` | v3.8.2 `IOT_2016_ixi` | not applied | 6,003.434509 | `01_/2016_uncorrected`, 6,005.239508 |
| `2019_uncorrected` | v3.8.2 `IOT_2016_ixi` | not applied | 6,307.954902 | `01_/2019_uncorrected`, 6,309.856788 |
| `2022_uncorrected` | v3.8.2 `IOT_2022_ixi` | not applied | 5,686.266020 | `01_/2022_uncorrected`, 5,688.078556 |
...
- `analysis.audit_consistency` C1: partition total 4,265.07 + self-supply loop 1.81
  = 4,266.88, which equals the manuscript grand total. **PASS.**
...
1 % (+0.52 % for climate, `uncertainty_audit.csv`), and only the MRIO block taken on
its own has median exactly 1. The *mean* is inflated by every parameter at once,

$$\frac{\mathbb{E}[f]}{f} = \frac{\sum_k f_k\,e^{\sigma_k^2/2}}{\sum_k f_k} = 1.00926$$

i.e. +0.93 % for climate, not the +0.35 % the MRIO factor supplies on its own; this
...
| `commute` | GSD 1.25 | direct estimate: TU workplace distance × residents aged 6 and over × health care's share of hours worked (NABB117) × the Dutch inventory's life-cycle intensity per person-km |
...
deterministic total is an **EXIOBASE** footprint of **4.267 Mt**, 50 % larger. The
...
| Quantity | Value |
|:---|:---|
| Deterministic climate | 4,267 kt |
| Median | 4,289 kt |
| 95 % interval | 3,675 to 5,029 kt |
| CV, Tier 2 simulation | 8.00 % |
| CV, Tier 1 error propagation | 8.04 % |
| MRIO share of variance | 69.6 % |

Both tiers now use the same per-term variance, $a_k^2(e^{\sigma_k^2}-1)e^{\sigma_k^2}$,
so the 0.04 percentage points between them is a matter of what each divides by: Tier 1
normalises on the deterministic total, the reported Tier 2 CV on the simulated mean,
which sits 0.89 % above it. Until 11 September 2026 Tier 1 used
...
- Variance shares sum to 100.0 % once the covariance of the correlated travel
  pair is carried as its own row; without it the own-terms reach only 86.6 %. That the
  ...
  (closed form 69.6 %, frozen-input 69.7 %) and the travel block including its covariance
  (closed form 30.28 %, frozen-input 30.14 %).
- `analysis.uncertainty_audit` runs nineteen numerical tests on the drawn samples and
  ...
  measures; and the MRIO factor, which carries 69.6 % of the variance, is included in
...
| This study, headline | 4.27 | 0.726 | 5.5 | 0.679 |
| + their sector boundary (NACE Q incl. childcare) | 4.99 | 0.850 | 6.5 | 0.795 |
| **+ their capital treatment (endogenised)** | **6.14** | **1.046** | **8.0** | **0.977** |

The ratio column is per capita. After both boundaries are matched the total sits
**0.7 % above** theirs (6.14 against 6.10 Mt) and the per-capita figure 2.3 % below
(1.046 against 1.070 t), with a national share of 7.95 % against their 8.3 %. The
apparent 30 % gap in the total is boundary; what remains is smaller than the difference
in model construction and reference year (2016 against 2022) could produce, so the
agreement should not be read as more than consistency. Two corrections moved the ladder
in September 2026. Moving the distribution margins to the trade industries (13
September) put the matched figures 4.8 % and 7.6 % below theirs; estimating commuting
directly, and giving the widest boundary its childcare operations in the direct
emissions, waste and commuting hours (14 September), raised the matched rung by
334 kt.
...
| Transport share of the supply-chain footprint | 36.8 % | **19.5 %** |
| DK sea transport as a producing node | 852 kt | **53 kt** |
| Danish national footprint | 85.2 Mt | **77.2 Mt** |

The 19.5 % is on the 3,415 kt MRIO supply-chain basis; on the 4,267 kt total, which
includes the entirely-Danish bottom-up items, transport is 15.6 %. **Quote the basis with
...
| Perspective | Gold table | Transport, climate | Share of 4,267 kt | Share of 3,415 kt MRIO |
|:---|:---|:---|:---|:---|
| Producing node (hotspot), summing over $j$ | `01_eriksen_replication/2022c/hotspot_by_sector_group.csv` | 664.9 kt | 15.58 % | 19.47 % |
| Purchased product (contribution), summing over $i$ | `01_eriksen_replication/2022c/contribution_by_sector_group.csv` | 566.5 kt | 13.28 % | 16.59 % |

Transport ranks **first** among the input-output model's producing-sector groups and
**second** among all producing groups, after private travel (707.7 kt, 16.59 %), and
**fourth** among purchased products, where the chemical group leads at 1,154.3 kt
(27.05 %). Any sentence that ranks transport
...
**2022 background** (applied $\phi = 0.0651$; the footprint is the full 4,267 kt total,
transport's share is on the purchased-product perspective):

| $\phi$ | Released, M€ | Footprint, kt CO₂-eq | Transport, % of total |
|---:|---:|---:|---:|
| 0.050 | 12,222.0 | 4,244.84 | 12.94 |
| 0.065 | 11,954.9 | 4,266.80 | 13.28 |
| **0.0651** | **11,953.9** | **4,266.88** | **13.28** |
| 0.077 | 11,741.2 | 4,284.59 | 13.55 |
| 0.090 | 11,509.8 | 4,304.08 | 13.84 |
| 0.100 | 11,331.7 | 4,319.22 | 14.07 |
| 0.125 | 10,886.6 | 4,357.71 | 14.64 |
| 0.150 | 10,441.5 | 4,397.11 | 15.22 |

**2016 background** (applied $\phi = 0.0774$, i.e. the 2019 analysis year):

| $\phi$ | Released, M€ | Footprint, kt CO₂-eq | Transport, % of total |
|---:|---:|---:|---:|
| 0.050 | 10,573.2 | 3,930.39 | 18.99 |
| 0.065 | 10,341.7 | 3,965.93 | 19.53 |
| 0.077 | 10,156.5 | 3,994.74 | 19.95 |
| **0.0774** | **10,151.0** | **3,995.59** | **19.97** |
| 0.090 | 9,955.9 | 4,026.31 | 20.41 |
| 0.100 | 9,801.6 | 4,050.87 | 20.77 |
| 0.125 | 9,415.8 | 4,113.32 | 21.64 |
| 0.150 | 9,029.9 | 4,177.30 | 22.52 |
...
2022 total by 152 kt, 3.6 %, and transport's share of it by 2.3 percentage points. The
choice between the published 0.09 and the table-read 0.0651 is worth 37 kt, 0.9 %. Second,
the band does not bridge to any earlier published total, because two further changes
intervene. Before the share was read per year the study published 4,712.42 kt for 2022,
which the $\phi = 0.09$ row reproduced exactly until 13 September 2026; the distribution
margins then moved to the Danish trade industries, which lowered every row of the band by
the same amount to within a few kilotonnes, and on 14 September 2026 the direct commuting
estimate raised every 2022 row by 104.76 kt (4,304.08 kt at $\phi = 0.09$). For 2019
the published figure on $\phi = 0.09$ was also characterised on IPCC AR4. Both steps are
quantified in
[docs/revision/defects_and_fixes.md](../revision/defects_and_fixes.md).
...
| Indicator | Change |
|:---|:---|
| Climate change | 3,534.9 → 4,285.8 kt, **+21.2 %** |
| Material extraction | +38.9 % |
| Blue water | +12.5 % |
| Land use | +22.5 % |
| Waste generation | +19.4 % |

The simplified construction used earlier gave +23.0 % (4,347.1 kt,
`capital_scenarios_by_indicator.csv`, `D_full_endogenisation`), so the published
method validates it to within 1.7 percentage points.
...
| Material extraction arising in Asia-Pacific | 25,243 of 33,801 kt, 74.68 % | p e953 |
...
entering the MRIO is much smaller than health expenditure: 13,054 M€ against
...
7.3 %), reproduces in Denmark (6.5 % against 5.5 %). That reproduction is an independent
...
current 4,266.9, and a cumulative production-layer share computed on a model
